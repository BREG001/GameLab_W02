...
public class TransparentGame : MonoBehaviour
{
    ...
    public void Update()
    {
        if (Camera.main.ScreenToWorldPoint(Input.mousePosition).y > -10.5f && !isTransparent)
        {
            if (toggle)
            {
                isTransparent = false;
        ...
        else if (Camera.main.ScreenToWorldPoint(Input.mousePosition).y <= -10.5f && isTransparent)
        {
            isTransparent = false;
            SetWindowLong(hWnd, GWL_EXSTYLE, WS_EX_LAYERED);
        }
